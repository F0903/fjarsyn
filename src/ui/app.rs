...
pub fn init(capture: Arc<RwLock<PlatformCaptureProvider>>) -> (State, Task<Message>) {
    ...
    let init_task = if onboarding_done {
        Task::future(async move {
            WebRTC::init(server_url, init_frame_tx, init_event_tx, ctx.config.max_depacket_latency)
                .await
        })
        .map_err(Arc::new)
...
pub fn subscription(state: &State) -> Subscription<Message> {
    ...
    let event_subscription = if let Some(rx) = &state.ctx.webrtc_event_rx {
        Subscription::run_with(WebRTCEventReceiverRef(rx.clone()), webrtc_event_subscription_stream)
    } else {
        Subscription::none()
    };
...
pub fn update(state: &mut State, message: Message) -> Task<Message> {
    fn delegate_to_screen(state: &mut State, msg: Message) -> Task<Message> {
        state.active_screen.update(&mut state.ctx, msg)
    }
