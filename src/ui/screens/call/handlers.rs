...
impl CallScreen {
    ...
    fn run_effects(&mut self, ctx: &mut AppState, effects: Vec<CallEffect>) -> Task<Message> {
        Task::batch(effects.into_iter().map(|effect| self.run_effect(ctx, effect)))
    }

    fn run_effect(&mut self, ctx: &mut AppState, effect: CallEffect) -> Task<Message> {
        match effect {
            ...
            CallEffect::InitializeCapture => self.perform_initialize_capture(ctx),
            ...
            CallEffect::EndCall => self.perform_end_call(ctx),
        }
    }
}
