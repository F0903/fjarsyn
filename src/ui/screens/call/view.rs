...
impl CallScreen {
    ...
    fn view_remote_video(&self, ctx: &AppState) -> Element<'_, Message> {
    ...
    fn view_waiting_state(&self, ctx: &AppState) -> Element<'_, Message> {
    ...
    fn waiting_state_copy(&self, ctx: &AppState) -> WaitingStateCopy {
    ...
    fn remote_name(&self, ctx: &AppState) -> String {
        ...
        peer_id
            .map(|peer_id| {
                format!("Remote Peer - ID {}", crate::utils::string_utils::truncate(peer_id, 8))
            })
            .unwrap_or_else(|| "Call in Progress".into())
    }

    fn control_specs(&self, ctx: &AppState) -> Vec<ControlSpec<'static>> {
