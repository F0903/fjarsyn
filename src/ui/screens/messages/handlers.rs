    ...
    pub(crate) fn handle_message(&mut self, ctx: &mut AppState, message: Message) -> Task<Message> {
        ...
        Task::batch(effects.into_iter().map(|effect| match effect {
            workflow::MessagesEffect::SendMessage { peer_id, body } => {
                match resolve_peer_address(ctx, &peer_id) {
                    Ok(address) => {
...
fn resolve_peer_address(ctx: &AppState, peer_id: &str) -> Result<SocketAddr, String> {
    ...
    if let Some(contacts) = ctx.services.contacts_service.as_ref()
