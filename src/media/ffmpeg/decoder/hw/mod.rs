use ffmpeg::{Codec, codec, decoder, frame};
use ffmpeg_next as ffmpeg;

use super::{DecoderInfo, FFmpegDecoderError, Result};
use crate::media::{ffmpeg::HWAccelType, frame::Frame, pixel_format::PixelFormat};

#[cfg(target_os = "windows")]
mod d3d11va;

pub(super) struct HwDecoderBackend {
    #[cfg(target_os = "windows")]
    kind: HwDecoderBackendKind,
}

#[cfg(target_os = "windows")]
enum HwDecoderBackendKind {
    D3d11va(d3d11va::D3d11vaBackend),
}

impl HwDecoderBackend {
    pub(super) fn open_decoder(
        codec: Codec,
        decoder_info: DecoderInfo,
    ) -> Result<(decoder::Video, Option<Self>)> {
        let mut context = codec::context::Context::new_with_codec(codec);
        context.set_flags(codec::Flags::LOW_DELAY);

        let mut hw_backend = Self::configure(&codec, &mut context, decoder_info.hw_accel);

        match context.decoder().open_as(codec).and_then(|d| d.video()) {
            Ok(decoder) => {
                if let Some(backend) = hw_backend.as_ref() {
                    tracing::info!(
                        "Using {} hardware decoding for {}",
                        backend.name(),
                        decoder_info.name
                    );
                }
                Ok((decoder, hw_backend))
            }
            Err(err) => {
                if let Some(backend_name) = hw_backend.as_ref().map(Self::name) {
                    tracing::warn!(
                        "Failed to open {} with {}, falling back to software decode: {}",
                        decoder_info.name,
                        backend_name,
                        err
                    );
                    let _ = hw_backend.take();

                    let mut context = codec::context::Context::new_with_codec(codec);
                    context.set_flags(codec::Flags::LOW_DELAY);
                    let decoder = context
                        .decoder()
                        .open_as(codec)
                        .and_then(|d| d.video())
                        .map_err(FFmpegDecoderError::Create)?;

                    return Ok((decoder, None));
                }

                Err(FFmpegDecoderError::Create(err))
            }
        }
    }

    #[cfg(target_os = "windows")]
    fn configure(
        codec: &Codec,
        context: &mut codec::Context,
        hw_accel: HWAccelType,
    ) -> Option<Self> {
        match hw_accel {
            HWAccelType::D3D11VA => d3d11va::D3d11vaBackend::configure(codec, context)
                .map(|backend| Self { kind: HwDecoderBackendKind::D3d11va(backend) }),
            HWAccelType::None => None,
        }
    }

    #[cfg(not(target_os = "windows"))]
    fn configure(
        _codec: &Codec,
        _context: &mut codec::Context,
        _hw_accel: HWAccelType,
    ) -> Option<Self> {
        None
    }

    pub(super) fn materialize_frame(&self, decoded_frame: frame::Video) -> Result<frame::Video> {
        #[cfg(target_os = "windows")]
        match &self.kind {
            HwDecoderBackendKind::D3d11va(backend) => backend.materialize_frame(decoded_frame),
        }

        #[cfg(not(target_os = "windows"))]
        {
            Ok(decoded_frame)
        }
    }

    pub(super) fn try_decode_frame(
        &self,
        decoded_frame: &frame::Video,
        dst_format: PixelFormat,
    ) -> Result<Option<Frame>> {
        #[cfg(target_os = "windows")]
        match &self.kind {
            HwDecoderBackendKind::D3d11va(backend) => {
                backend.try_decode_frame(decoded_frame, dst_format)
            }
        }

        #[cfg(not(target_os = "windows"))]
        {
            let _ = (decoded_frame, dst_format);
            Ok(None)
        }
    }

    fn name(&self) -> &'static str {
        #[cfg(target_os = "windows")]
        match &self.kind {
            HwDecoderBackendKind::D3d11va(backend) => backend.name(),
        }

        #[cfg(not(target_os = "windows"))]
        {
            "software"
        }
    }
}
