...
pub(in crate::ui) struct CpuFrameViewer {
    frame: Arc<Frame>,
}
    ...
    fn layout(
        ...
        if state.frame_id != frame_id {
            ...
            state.image_handle = if self.frame.format.is_iced_compatible() {
                self.frame.software_pixels().map(|pixels| {
                    advanced::image::Handle::from_rgba(
                        self.frame.size.width as u32,
                        self.frame.size.height as u32,
                        pixels,
                    )
                })
            } else {
        ...
        let src_width = self.frame.size.width as f32;
        let src_height = self.frame.size.height as f32;
