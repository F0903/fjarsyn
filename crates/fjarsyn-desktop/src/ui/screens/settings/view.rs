...
use crate::ui::{
    fonts,
    message::{self, Message, screen::settings::Message as SettingsMessage},
...
impl Screen {
    fn unsaved_changes_bar(&self) -> Element<'_, Message> {
        ...
        .padding([6, 9])
        .on_press(Message::Screen(message::Screen::Settings(SettingsMessage::SaveSettings)))
        ...
        .padding([6, 9])
        .on_press(Message::Screen(message::Screen::Settings(SettingsMessage::DiscardSettings)))
        ...
        container(
            row![
                row![
                    ...
                    text("Unsaved changes").size(SIZE).font(fonts::BOLD)
                ]
                .spacing(6),
                rule::vertical(1),
    ...
    fn view_sidebar(&self) -> Element<'_, Message> {
        let tabs = tabs::iter().map(|tab| {
            let is_active = self.active_tab.id() == tab.id();
            button(
            ...
            .width(Length::Fill)
            .padding(12)
            .on_press(Message::Screen(message::Screen::Settings(SettingsMessage::TabChanged(
                tab.id(),
            ))))
        ...
        container(
            column![
                text("Categories").size(12).font(fonts::SEMIBOLD).style(text::secondary),
