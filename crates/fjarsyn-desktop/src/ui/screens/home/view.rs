...
impl Screen {
    pub(in crate::ui::screens) fn render_view<'a>(
        ...
        let nearby_contacts = context
            ...
            .iter()
            .filter_map(|contact| {
                context.is_nearby(&contact.peer_id).then_some((contact, contact.peer_id.clone()))
            })
            .collect::<Vec<_>>();
        ...
        if nearby_contacts.is_empty() {
            ...
            for (contact, peer_id) in nearby_contacts {
                let session = context.session_for_peer(&peer_id);
                let session_label = match session.map(|session| session.phase) {
                    Some(Phase::Requesting | Phase::Negotiating) => "Connecting",
                    Some(Phase::Incoming) => "Incoming request",
                    Some(Phase::Connected) => "Connected",
                    Some(Phase::Reconnecting) => "Reconnecting",
                    Some(Phase::Disconnecting) => "Disconnecting",
                    None => "Not connected",
                };
        ...
        let content = column![
            column![
                text("Home").size(32).style(text::primary).font(fonts::BOLD),
