use fjarsyn_engine::{contacts::Contact, pairing::Invite, peer_session::Phase};
use iced::{
...
use crate::ui::{
    fonts,
    message::{self, Message, Route, screen::contacts::Message as ContactsMessage},
    presentation::{Context, fingerprint_grid, project_peer},
    theme,
};
...
impl Screen {
    pub(super) fn render_view<'a>(&'a self, context: Context<'a>) -> Element<'a, Message> {
        ...
        if self.show_add_form {
            content = content.push(self.view_add_contact_form(context.local_peer_id().is_some()));
        }

        content = content.push(container(self.view_contacts_list(context)).width(Length::Fill));
    ...
    fn view_header(&self) -> Element<'_, Message> {
        ...
        if self.saving_new_contact.is_none() {
            form_button = form_button.on_press(Message::Screen(message::Screen::Contacts(
        ...
        row![
            column![
                text("Contacts").size(32).style(text::primary).font(fonts::BOLD),
    ...
    fn view_add_contact_form(&self, local_identity_ready: bool) -> Element<'_, Message> {
        ...
        if self.saving_new_contact.is_none() {
            invite_input = invite_input.on_input(|value| {
                Message::Screen(message::Screen::Contacts(ContactsMessage::NewInviteChanged(value)))
            });
        }
        ...
        if !reading && self.saving_new_contact.is_none() {
            paste_button = paste_button.on_press(Message::Screen(message::Screen::Contacts(
        ...
        if can_save {
            save_button = save_button.on_press(Message::Screen(message::Screen::Contacts(
        ...
        if self.saving_new_contact.is_none() {
            name_input = name_input.on_input(|value| {
                Message::Screen(message::Screen::Contacts(ContactsMessage::NameChanged(value)))
            });
        }
    ...
    fn view_new_pairing_verification(&self) -> Element<'_, Message> {
        ...
        if self.saving_new_contact.is_none() {
            confirmation = confirmation.on_toggle(|confirmed| {
                Message::Screen(message::Screen::Contacts(
    ...
    fn view_contacts_list<'a>(&'a self, context: Context<'a>) -> Element<'a, Message> {
        ...
        if context.contacts().is_empty() {
            ...
            for contact in context.contacts() {
                ...
                let nearby = context.is_nearby(&contact_peer_id);
                let phase = context.session_for_peer(&contact_peer_id).map(|session| session.phase);
                ...
                let session_label = phase
                    .map(|phase| match phase {
                        Phase::Requesting | Phase::Negotiating => "Connecting",
                ...
                .on_press(Message::Navigation(message::Navigation::Navigate(Route::Peer {
                ...
                let delete_actions: Element<'_, Message> = if delete_requested {
                    ...
                    if can_delete && !delete_in_flight {
                        confirm = confirm.on_press(Message::Screen(message::Screen::Contacts(
                    ...
                    let mut cancel = button(text("Cancel").size(12))
                        ...
                        .style(|theme, status| theme::button_style(theme, status, false));
                    if !delete_in_flight {
                        cancel = cancel.on_press(Message::Screen(message::Screen::Contacts(
                    ...
                    if can_delete {
                        delete_button =
                            delete_button.on_press(Message::Screen(message::Screen::Contacts(
    ...
    fn view_identity_editor<'a>(
            ...
            .filter(|replacement| replacement.contact_id == contact.id)
        {
            ...
            if replacement.saving.is_none() {
                cancel_button = cancel_button.on_press(Message::Screen(message::Screen::Contacts(
        ...
        if trust_mutation_allowed && self.identity_replacement.is_none() {
            replace_button = replace_button.on_press(Message::Screen(message::Screen::Contacts(
...
fn pairing_preview(invite: &Invite) -> Element<'_, Message> {
...
fn stored_fingerprint(contact: &Contact) -> String {
    Invite::new(contact.peer_id.clone(), contact.trusted_public_key.clone())
