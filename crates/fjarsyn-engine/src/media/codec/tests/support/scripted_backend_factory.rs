...
use crate::media::{
    codec::{
        ...
        backend::{BackendFactory, DecoderBackend, EncoderBackend},
    },
    frame::Frame,
};

#[derive(Clone)]
pub(in crate::media::codec::tests) enum EncoderPlan {
    CreateError,
...
#[derive(Clone)]
pub(in crate::media::codec::tests) enum DecoderPlan {
    Pass,
    Block(BlockingGate),
}

#[derive(Clone)]
pub(in crate::media::codec::tests) struct ScriptedBackendFactory {
    encoders: Arc<Mutex<VecDeque<EncoderPlan>>>,
    decoders: Arc<Mutex<VecDeque<DecoderPlan>>>,
}

impl ScriptedBackendFactory {
    pub(in crate::media::codec::tests) fn new(
