...
use super::{
    ...
    negotiation::{NegotiationConnection, NegotiationService},
    protocol::{ControlMessage, DATA_PROTOCOL_VERSION, MessagingMessage, NegotiationSignal},
    restart::{IceRestartCoordinator, TransportGeneration},
...
const MAX_RESTART_REJECTION_TASKS: usize = 4;
...
pub(crate) enum SessionCommand {
    ...
    #[cfg(test)]
    ForceIceRestart(oneshot::Sender<Result<(), PeerSessionError>>),
    #[cfg(test)]
    CommittedTransportGeneration(oneshot::Sender<Result<TransportGeneration, PeerSessionError>>),
}
...
#[derive(Debug)]
pub(crate) struct RestartAttachment {
...
pub(crate) struct SessionActorHandle {
    ...
    restart_tx: mpsc::Sender<RestartAttachment>,
    snapshot_rx: watch::Receiver<PeerSessionSnapshot>,
    encoded_video_tx: mpsc::Sender<super::media::OutboundVideoSample>,
    active_video_rx: watch::Receiver<Option<(ShareId, ShareEpoch)>>,
    remote_video_tx: broadcast::Sender<super::media::RemoteVideoSample>,
    initial_remote_video_rx:
        Arc<Mutex<Option<broadcast::Receiver<super::media::RemoteVideoSample>>>>,
...
impl SessionActorHandle {
    ...
    pub(crate) fn try_attach_restart(
        &self,
        attachment: RestartAttachment,
    ) -> Result<(), Box<RestartAttachment>> {
    ...
    pub(crate) fn encoded_video_sink(
        &self,
        share_id: ShareId,
        ...
        match self.snapshot_rx.borrow().local_share {
            ...
            _ => Err(PeerSessionError::ShareMismatch(share_id)),
        }
    }
...
pub(crate) fn spawn(
    ...
    let actor = SessionActor {
        ...
        restart: IceRestartCoordinator::default(),
        restart_sdp_applied: false,
        old_transport_recovery_allowed: true,
        restart_result_tx,
        restart_result_rx,
        restart_dial_task: None,
        restart_rx,
        restart_channel_open: true,
        restart_connection_origin: None,
        restart_rejection_tasks: JoinSet::new(),
...
struct SessionActor {
    ...
    last_local_share_epoch: Option<ShareEpoch>,
    last_remote_share_epoch: Option<ShareEpoch>,
    connection: Option<NegotiationConnection>,
    rtc: Option<RtcPeer>,
    command_rx: mpsc::Receiver<SessionCommand>,
    video_rx: mpsc::Receiver<super::media::OutboundVideoSample>,
    active_video_tx: watch::Sender<Option<(ShareId, ShareEpoch)>>,
    remote_video_tx: broadcast::Sender<super::media::RemoteVideoSample>,
    ...
    restart: IceRestartCoordinator,
    restart_sdp_applied: bool,
    old_transport_recovery_allowed: bool,
    restart_result_tx: mpsc::Sender<RestartDialResult>,
    restart_result_rx: mpsc::Receiver<RestartDialResult>,
    restart_dial_task: Option<JoinHandle<()>>,
    restart_rx: mpsc::Receiver<RestartAttachment>,
    restart_channel_open: bool,
    restart_connection_origin: Option<RestartConnectionOrigin>,
    restart_rejection_tasks: JoinSet<()>,
    cleanup_deadline: Option<Instant>,
    pending_application_data: VecDeque<(ChannelKind, bytes::Bytes)>,
}

struct RestartDialResult {
    generation: TransportGeneration,
    result: Result<NegotiationConnection, PeerSessionError>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum RestartConnectionOrigin {
    Dialed,
    Attached,
}
...
impl SessionActor {
    async fn run(mut self) {
        ...
        while self.terminal_reason.is_none() {
            let restart_deadline = self
                .restart
                .active()
                .map(|attempt| attempt.deadline())
                .filter(|_| self.state.phase() == PeerSessionPhase::Reconnecting);
            tokio::select! {
                biased;
                ...
                _ = wait_for_deadline(restart_deadline) => {
                    self.fail("ICE restart timed out".into());
                }
                joined = self.restart_rejection_tasks.join_next(),
                    if !self.restart_rejection_tasks.is_empty() =>
                {
                    if joined.is_some_and(|result| result.is_err()) {
                        self.fail("restart rejection cleanup task failed".into());
                    }
                }
                attachment = self.restart_rx.recv(), if self.restart_channel_open => {
                    match attachment {
                        Some(RestartAttachment { generation, connection }) => {
                            self.attach_restart(generation, connection).await;
                        }
                        None => self.restart_channel_open = false,
                    }
                }
                dial = self.restart_result_rx.recv(), if self.restart_dial_task.is_some() => {
                    if let Some(task) = self.restart_dial_task.take() {
                        let _ = task.await;
                    }
                    match dial {
                        Some(result) => self.handle_restart_dial_result(result).await,
                        None => self.fail("ICE restart signaling task stopped".into()),
                    }
                }
                signal = async {
                ...
                tagged = self.video_rx.recv() => {
                    ...
                    if self.local_share == (LocalShareState::Active {
                        ...
                        && self.state.phase() == PeerSessionPhase::Connected
                ...
                _ = timeout_check.tick() => {
                    let timeout = match self.state.phase() {
                        ...
                        | PeerSessionPhase::Disconnecting => None,
                    };
        ...
        self.abort_restart_dial_and_drain(cleanup_deadline).await;
        self.restart_rejection_tasks.abort_all();
        while !self.restart_rejection_tasks.is_empty()
            && tokio::time::timeout_at(cleanup_deadline, self.restart_rejection_tasks.join_next())
                .await
                .is_ok()
        {}
        while let Ok(attachment) = self.restart_rx.try_recv() {
            attachment.connection.shutdown_until(cleanup_deadline).await;
        }
        let connection = self.connection.take();
        self.restart_connection_origin = None;
    ...
    async fn handle_command(&mut self, command: SessionCommand) {
        match command {
            ...
            #[cfg(test)]
            SessionCommand::ForceIceRestart(reply) => {
            ...
            #[cfg(test)]
            SessionCommand::CommittedTransportGeneration(reply) => {
    ...
    async fn stop_share(&mut self, share_id: ShareId) -> Result<(), PeerSessionError> {
        ...
        let epoch = match self.local_share {
            ...
            _ => return Err(PeerSessionError::ShareMismatch(share_id)),
        };
    ...
    async fn begin_ice_restart(&mut self) -> Result<(), PeerSessionError> {
        self.begin_ice_restart_with_old_transport_recovery(true).await
    }

    async fn begin_ice_restart_with_old_transport_recovery(
        &mut self,
        allowed: bool,
    ) -> Result<(), PeerSessionError> {
        if self.state.phase() == PeerSessionPhase::Reconnecting {
            return Ok(());
        }
        if self.state.phase() != PeerSessionPhase::Connected {
        ...
        let generation = self.restart.begin_local(deadline)?;
        self.old_transport_recovery_allowed = allowed;
        self.apply(SessionInput::TransportLost).await?;
        self.reset_restart_readiness();
        self.disconnected_since = None;

        self.spawn_restart_dial(generation, deadline);
        Ok(())
    }

    fn spawn_restart_dial(&mut self, generation: TransportGeneration, deadline: Instant) {
        debug_assert!(self.restart_dial_task.is_none());
        let negotiation = self.config.negotiation.clone();
        let session_id = self.config.session_id;
        let peer_id = self.config.remote_peer_id.clone();
        let result_tx = self.restart_result_tx.clone();
        self.restart_dial_task = Some(tokio::spawn(async move {
            let result =
                tokio::time::timeout_at(deadline, negotiation.connect(session_id, peer_id))
                    .await
                    .map_err(|_| {
                        PeerSessionError::Signaling("ICE restart signaling timed out".into())
                    })
                    .and_then(|result| result);
            let _ = result_tx.send(RestartDialResult { generation, result }).await;
        }));
    }

    async fn handle_restart_dial_result(&mut self, dial: RestartDialResult) {
        let active_generation = self.restart.active().map(|attempt| attempt.generation());
        if self.state.phase() != PeerSessionPhase::Reconnecting
            || active_generation != Some(dial.generation)
        {
        ...
        match dial.result {
            Ok(connection) => {
                ...
                self.restart_connection_origin = Some(RestartConnectionOrigin::Dialed);
            }
            Err(error) => self.fail_error(error),
        }
    }

    async fn attach_restart(
        &mut self,
        generation: TransportGeneration,
        ...
        let active = self.restart.active();
        let conflicts_with_canonical_connection = self.restart_connection_origin
            == Some(RestartConnectionOrigin::Attached)
            || (self.config.role == SessionRole::Outgoing
                && (self.restart_dial_task.is_some()
                    || self.restart_connection_origin == Some(RestartConnectionOrigin::Dialed)))
            || (self.config.role == SessionRole::Incoming
                && self.restart_connection_origin == Some(RestartConnectionOrigin::Dialed)
                && active.is_some_and(|attempt| attempt.authorized()));
        if conflicts_with_canonical_connection {
            self.reject_restart_connection(connection, "another restart signaling path won");
            return;
        }

        let newly_started = self.state.phase() == PeerSessionPhase::Connected;
        let admission = if newly_started {
            let deadline = Instant::now() + self.config.ice_restart_timeout;
            match self.restart.begin_remote(generation, deadline) {
                Ok(()) => self.apply(SessionInput::TransportLost).await,
                Err(error) => Err(error),
            }
        } else {
        ...
        if self.config.role == SessionRole::Incoming {
            self.abort_restart_dial_and_drain(deadline).await;
            if self.restart_connection_origin == Some(RestartConnectionOrigin::Dialed) {
        ...
        self.connection = Some(connection);
        self.restart_connection_origin = Some(RestartConnectionOrigin::Attached);
        if self.config.role == SessionRole::Outgoing
    ...
    fn reject_restart_connection(
        ...
        if self.restart_rejection_tasks.len() >= MAX_RESTART_REJECTION_TASKS {
            return;
        }
        let deadline = self
            .restart
            .active()
            .map(|attempt| attempt.deadline())
            .unwrap_or_else(|| Instant::now() + self.config.cleanup_timeout)
            .min(Instant::now() + self.config.cleanup_timeout);
        let reason = reason.into();
        self.restart_rejection_tasks.spawn(async move {
    ...
    async fn handle_restart_ack(
        ...
        if attempt.authorized() {
            return Err(PeerSessionError::Protocol("duplicate ICE restart acknowledgement".into()));
        }
    ...
    fn reset_restart_readiness(&mut self) {
        self.pc_connected = false;
        self.local_transport_ready = false;
        self.remote_ready = false;
        self.ready_acknowledged = false;
        self.restart_sdp_applied = false;
    }

    fn require_transport_generation(
        &self,
        generation: TransportGeneration,
    ) -> Result<(), PeerSessionError> {
        let expected = match self.state.phase() {
            PeerSessionPhase::Negotiating => self.restart.committed(),
            PeerSessionPhase::Reconnecting => self
                .restart
                .active()
                .ok_or_else(|| PeerSessionError::Protocol("missing ICE restart attempt".into()))?
                .generation(),
            _ => {
    ...
    async fn handle_signal(&mut self, signal: NegotiationSignal) {
        let result = match signal {
            ...
            NegotiationSignal::Restart { .. } => Err(PeerSessionError::Protocol(
                "restart intent received after signaling routing".into(),
            )),
    ...
    async fn handle_offer(
        ...
        let restart_deadline = if phase == PeerSessionPhase::Reconnecting {
            let attempt = self.restart.require_active(generation)?;
            if !attempt.authorized() {
                return Err(PeerSessionError::Protocol(
        ...
        let answer = match phase {
            PeerSessionPhase::Negotiating => rtc.apply_offer_and_create_answer(sdp).await?,
            PeerSessionPhase::Reconnecting => tokio::time::timeout_at(
        ...
        if phase == PeerSessionPhase::Reconnecting {
            self.restart_sdp_applied = true;
        }
    ...
    async fn handle_answer(
        ...
        let applied = match phase {
            PeerSessionPhase::Negotiating => rtc.apply_answer(sdp).await,
            PeerSessionPhase::Reconnecting => tokio::time::timeout_at(
                restart_deadline.expect("restart deadline checked above"),
    ...
    async fn handle_rtc_event(&mut self, event: RtcEvent) {
        let result = match event {
            RtcEvent::LocalCandidate { generation, candidate } => {
                if should_forward_local_candidate(
                    ...
                    let prepared = self
                        ...
                        .as_ref()
                        .ok_or_else(|| {
                            PeerSessionError::WebRtc("peer connection is unavailable".into())
                        })
            ...
            RtcEvent::IceState { generation, state } => {
                self.handle_ice_state(generation, state).await
            }
            RtcEvent::DtlsState { generation, state } => {
            ...
            RtcEvent::PeerState { generation, state } => {
                if !self.transport_event_is_relevant(generation)
        ...
        match state {
            ...
            RTCIceConnectionState::Failed => {
                ...
                match self.state.phase() {
                    PeerSessionPhase::Connected => self.begin_ice_restart().await,
                    PeerSessionPhase::Negotiating => {
                        self.fail("ICE failed during negotiation".into());
                        Ok(())
                    }
                    PeerSessionPhase::Reconnecting => Ok(()),
                    _ => Ok(()),
                }
            }
    ...
    async fn handle_transport_connected(
            ...
            && self.old_transport_recovery_allowed
        {
            let deadline = self
                ...
                .deadline();
            let transport_is_operational = self.control_open
                && self.messaging_open
    ...
    async fn cancel_unengaged_restart(&mut self) -> Result<(), PeerSessionError> {
        let deadline = self
            .restart
            .active()
            .ok_or_else(|| PeerSessionError::Protocol("missing ICE restart attempt".into()))?
        ...
        if Instant::now() >= deadline {
            return Err(PeerSessionError::OperationTimeout);
        }
        ...
        if !operational {
            return Err(PeerSessionError::WebRtc(
                "transport was lost while cancelling ICE recovery".into(),
            ));
        }
        self.restart.cancel()?;
        ...
        self.apply(SessionInput::TransportRecovered).await?;
        self.flush_pending_application_data().await
    }

    async fn abort_restart_dial_and_drain(&mut self, deadline: Instant) {
        if let Some(task) = self.restart_dial_task.take() {
            task.abort();
            let _ = task.await;
        }
        while let Ok(result) = self.restart_result_rx.try_recv() {
            if let Ok(connection) = result.result {
                connection.shutdown_until(deadline).await;
            }
        }
    }
    ...
    fn transport_generation_is_current(&self, generation: TransportGeneration) -> bool {
        match self.state.phase() {
            PeerSessionPhase::Reconnecting => {
                self.restart.active().is_some_and(|attempt| attempt.generation() == generation)
            }
            _ => self.restart.committed() == generation,
        }
    }

    fn transport_event_is_relevant(&self, generation: TransportGeneration) -> bool {
        self.restart.committed() == generation
            || self.restart.active().is_some_and(|attempt| attempt.generation() == generation)
    }
    ...
    async fn route_channel_message(
        ...
        let is_disconnect = if matches!(
            self.state.phase(),
            PeerSessionPhase::Negotiating | PeerSessionPhase::Reconnecting
    ...
    async fn try_finish_ready(&mut self) -> Result<(), PeerSessionError> {
        if matches!(
            ...
            && self.ready_acknowledged
        {
            let initial = self.state.phase() == PeerSessionPhase::Negotiating;
    ...
    fn active_transport_generation(&self) -> Result<TransportGeneration, PeerSessionError> {
        match self.state.phase() {
            PeerSessionPhase::Negotiating => Ok(self.restart.committed()),
            PeerSessionPhase::Reconnecting => self
                .restart
                .active()
            ...
            _ => Err(PeerSessionError::Protocol(
                "session has no active transport negotiation".into(),
            )),
        }
    }
    ...
    async fn send_signal(&self, signal: NegotiationSignal) -> Result<(), PeerSessionError> {
        ...
        if let Some(attempt) = self.restart.active()
            && self.state.phase() == PeerSessionPhase::Reconnecting
        {
...
fn should_forward_local_candidate(
    ...
    matches!(phase, PeerSessionPhase::Negotiating | PeerSessionPhase::Reconnecting)
        && signaling_open
        && current_generation
}
...
fn application_data_disposition(
    ...
    match phase {
        ...
        PeerSessionPhase::Reconnecting if is_disconnect => ApplicationDataDisposition::Deliver,
        PeerSessionPhase::Reconnecting => ApplicationDataDisposition::Buffer,
...
fn require_connected_application_data(phase: PeerSessionPhase) -> Result<(), PeerSessionError> {
    if matches!(phase, PeerSessionPhase::Connected | PeerSessionPhase::Reconnecting) {
        Ok(())
    } else {
...
fn next_share_epoch(previous: Option<ShareEpoch>) -> Result<ShareEpoch, PeerSessionError> {
    match previous {
...
mod tests {
    ...
    fn local_candidates_are_only_forwarded_during_open_negotiation() {
        assert!(should_forward_local_candidate(PeerSessionPhase::Negotiating, true, true,));
        assert!(!should_forward_local_candidate(PeerSessionPhase::Negotiating, false, true,));
        assert!(!should_forward_local_candidate(PeerSessionPhase::Connected, false, true,));
        assert!(!should_forward_local_candidate(PeerSessionPhase::Connected, true, true,));
        assert!(should_forward_local_candidate(PeerSessionPhase::Reconnecting, true, true,));
        assert!(!should_forward_local_candidate(PeerSessionPhase::Reconnecting, true, false,));
    }
    ...
    fn application_data_racing_the_final_ready_ack_is_buffered_in_order() {
        ...
        assert_eq!(
            application_data_disposition(PeerSessionPhase::Reconnecting, false, false, false),
            ApplicationDataDisposition::Buffer
        );
        assert_eq!(
            application_data_disposition(PeerSessionPhase::Reconnecting, true, false, false),
            ApplicationDataDisposition::Deliver
        );
    ...
    async fn first_remote_video_source_keeps_samples_sent_before_subscription() {
        ...
        remote_video_tx
            .send(super::super::media::RemoteVideoSample { epoch, sample: sample.clone() })
            .unwrap();
