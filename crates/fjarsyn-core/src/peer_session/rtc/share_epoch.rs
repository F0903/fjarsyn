...
use super::super::{PeerSessionError, ShareEpoch};
...
pub(super) fn register(media_engine: &mut MediaEngine) -> Result<(), PeerSessionError> {
    media_engine
...
pub(super) fn outbound(epoch: ShareEpoch) -> Result<HeaderExtension, PeerSessionError> {
    epoch.require_valid()?;
...
pub(super) fn negotiated_id(parameters: &RTCRtpParameters) -> Result<u8, PeerSessionError> {
    ...
    let extension = matches.next().ok_or_else(|| {
        PeerSessionError::Protocol("screen-share epoch RTP extension was not negotiated".into())
    })?;
...
fn checked_extension_id(
    extension: &RTCRtpHeaderExtensionParameters,
) -> Result<u8, PeerSessionError> {
    let id = u8::try_from(extension.id).map_err(|_| {
        PeerSessionError::Protocol("screen-share epoch RTP extension ID is invalid".into())
    })?;
    if !(1..=14).contains(&id) {
        return Err(PeerSessionError::Protocol(
...
pub(super) fn decode(header: &Header, extension_id: u8) -> Result<ShareEpoch, PeerSessionError> {
    if !header.extension {
        return Err(PeerSessionError::Protocol(
            "RTP packet has no screen-share epoch extension".into(),
        ));
    }
    let mut matches = header.extensions.iter().filter(|extension| extension.id == extension_id);
    let extension = matches.next().ok_or_else(|| {
        PeerSessionError::Protocol("RTP packet has no screen-share epoch extension".into())
    })?;
    if matches.next().is_some() {
        return Err(PeerSessionError::Protocol(
    ...
    let bytes: [u8; WIRE_SIZE] = extension.payload.as_ref().try_into().map_err(|_| {
        PeerSessionError::Protocol("RTP packet has a malformed screen-share epoch".into())
    })?;
...
pub(super) fn video_sdp_id(sdp: &str) -> Result<u8, PeerSessionError> {
    ...
    negotiated_id.ok_or_else(|| {
        PeerSessionError::Protocol("video SDP has no screen-share epoch RTP extension".into())
    })
}
