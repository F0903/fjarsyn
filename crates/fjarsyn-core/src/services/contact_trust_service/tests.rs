...
use super::{ContactTrustError, ContactTrustService, PeerAdmissionControl};
use crate::{
    Error,
    database::ContactModel,
    identity::LocalPeerIdentity,
    pairing::{PairingInvite, VerifiedPeerIdentity},
    peer_session::{
        PeerEndpointResolver, PeerId, PeerSessionError, PeerSessionService,
        PeerSessionServiceConfig, TrustBarrierOwnerId,
    },
...
impl TestContactsStore {
    fn new(contact: ContactModel) -> Self {
...
#[async_trait]
impl PeerAdmissionControl for CancellationAdmissionControl {
...
#[async_trait]
impl PeerAdmissionControl for TestAdmissionControl {
    async fn ensure_trust_suspended(
        &self,
        peer_id: PeerId,
        owner_id: TrustBarrierOwnerId,
...
#[async_trait]
impl ContactsStore for TestContactsStore {
    async fn list(&self) -> Result<Vec<ContactModel>, Error> {
        if self.fail_list.swap(false, Ordering::SeqCst) {
            return Err(Error::InvalidContact("forced reconciliation failure".into()));
        }
    ...
    async fn delete(&self, id: i64) -> Result<(), Error> {
        ...
        if contacts.len() == before {
            return Err(Error::RecordNotFound { entity: "contact", id });
        }
        drop(contacts);
        if self.fail_delete_after_commit.swap(false, Ordering::SeqCst) {
            return Err(Error::InvalidContact("forced post-commit delete failure".into()));
        }
        Ok(())
    }
    ...
    async fn update(
        ...
        if self.fail_update.load(Ordering::SeqCst) {
            return Err(Error::InvalidContact("forced update failure".into()));
        }
        ...
        let contact = contacts
            ...
            .ok_or(Error::RecordNotFound { entity: "contact", id })?;
        ...
        if self.fail_update_after_commit.swap(false, Ordering::SeqCst) {
            return Err(Error::InvalidContact("forced post-commit update failure".into()));
        }
        Ok(committed)
    }
}
...
#[async_trait]
impl PeerEndpointResolver for NoEndpoints {
    async fn endpoint_hints_for(
        &self,
        _peer_id: &PeerId,
    ) -> Result<Arc<[std::net::SocketAddr]>, PeerSessionError> {
        Ok(Arc::from([]))
    }
}

fn contact_model(peer_id: &PeerId, public_key: String) -> ContactModel {
    let now = Utc::now();
    ContactModel {
        id: 1,
...
fn verified_identity(peer_id: &PeerId) -> VerifiedPeerIdentity {
    PairingInvite::new(peer_id.clone(), LocalPeerIdentity::generate().public_key_base64())
        .unwrap()
        .confirm()
}
...
) -> (PeerSessionService, ContactTrustService, PeerId, PeerId) {
    let remote_peer = PeerId::new("remote").unwrap();
    let local_peer = PeerId::new("local").unwrap();
    let contacts = Arc::new(ContactsService::new(store));
    contacts.refresh().await.unwrap();
    let mut config = PeerSessionServiceConfig::new(contacts.clone(), Arc::new(NoEndpoints));
    config.local_peer_id = Some(local_peer.clone());
    let sessions = PeerSessionService::start(config).await.unwrap();
    let coordinator = ContactTrustService::new(contacts, sessions.handle(), local_peer.clone());
...
async fn successful_delete_releases_the_barrier_after_removing_trust() {
    ...
    let (sessions, coordinator, remote_peer, _) = start_coordinator(store).await;

    assert!(coordinator.delete(1).await.unwrap().projection.contacts.is_empty());
    assert_eq!(
        sessions.handle().connect(remote_peer.clone()).await,
...
async fn committed_delete_returns_empty_projection_when_admission_recovery_is_unavailable() {
    ...
    let (sessions, coordinator, _, _) = start_coordinator(store.clone()).await;
    ...
    assert!(outcome.projection.contacts.is_empty());
    assert_eq!(outcome.admission_warning, Some(PeerSessionError::ServiceStopped));
}
...
async fn failed_key_update_restores_peer_admission() {
    ...
    assert!(matches!(result, Err(ContactTrustError::Contact(_))));
    assert_eq!(
        sessions.handle().connect(remote_peer.clone()).await,
        Err(PeerSessionError::PeerNotNearby(remote_peer))
    );
...
async fn key_update_is_bracketed_by_suspension_until_persistence_finishes() {
    ...
    let (sessions, coordinator, remote_peer, _) = start_coordinator(store.clone()).await;
...
async fn cancelled_acquire_keeps_recorded_intent_and_retry_reasserts_the_barrier() {
    ...
    let contacts = Arc::new(ContactsService::new(store.clone()));
    contacts.refresh().await.unwrap();
    let admission = Arc::new(CancellationAdmissionControl::default());
    let coordinator =
        ContactTrustService::new_with_admission_control(contacts, admission.clone(), local_peer);
...
async fn local_identity_is_rejected_before_persistence_or_session_mutation() {
    ...
    let (sessions, coordinator, _, local_peer) = start_coordinator(store.clone()).await;
    let snapshot_before = sessions.handle().snapshot();

    let result = coordinator.create("Myself".into(), verified_identity(&local_peer)).await;

    assert!(matches!(
        result,
        Err(ContactTrustError::SelfIdentity { peer_id }) if peer_id == local_peer
    ));
    assert_eq!(store.create_calls.load(Ordering::SeqCst), 0);
    assert_eq!(sessions.handle().snapshot(), snapshot_before);
...
async fn replacement_identity_must_belong_to_the_contacts_existing_peer() {
    ...
    assert!(matches!(
        result,
        Err(ContactTrustError::PeerIdentityMismatch {
            contact_id: 1,
            expected,
            actual,
...
async fn indeterminate_post_commit_update_stays_suspended_instead_of_trusting_the_stale_key() {
    ...
    assert!(matches!(result, Err(ContactTrustError::OutcomeUnknown { .. })));
    assert_eq!(
        sessions.handle().connect(remote_peer.clone()).await,
        Err(PeerSessionError::PeerSuspended(remote_peer.clone()))
    );
...
async fn authoritative_refresh_releases_a_definitively_applied_rotation_barrier() {
    ...
    assert!(matches!(
        ...
        Err(ContactTrustError::OutcomeUnknown { .. })
    ));
...
async fn refresh_surfaces_every_retained_barrier_and_retries_their_recovery() {
    ...
    let mut warned_peers = warning_outcome
        ...
        .iter()
        .map(|warning| {
            assert_eq!(warning.error, PeerSessionError::OperationTimeout);
...
async fn indeterminate_committed_delete_can_be_retried_without_double_suspension() {
    ...
    assert!(matches!(coordinator.delete(1).await, Err(ContactTrustError::OutcomeUnknown { .. })));
    assert_eq!(
        sessions.handle().connect(remote_peer.clone()).await,
        Err(PeerSessionError::PeerSuspended(remote_peer.clone()))
    );
