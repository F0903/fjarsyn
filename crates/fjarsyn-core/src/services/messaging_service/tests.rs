...
use super::*;
use crate::peer_session::{LocalShareState, PeerSessionSnapshot, RemoteShareState};
...
impl FakeMessagesStore {
    fn models(&self) -> Vec<MessageModel> {
        self.state.lock().unwrap().messages.clone()
    }

    fn insert_model(&self, model: MessageModel) {
...
impl MessagesStore for FakeMessagesStore {
    ...
    ) -> Result<MessageModel, crate::Error> {
        let mut state = self.state.lock().unwrap();
        state.next_id += 1;
    ...
    async fn mark_all_pending_unknown(&self) -> Result<Vec<MessageModel>, crate::Error> {
        let mut state = self.state.lock().unwrap();
...
struct FakeSessions {
    snapshot: PeerSessionServiceSnapshot,
...
impl SessionMessaging for FakeSessions {
    ...
    async fn send_message(
        ...
        if self.unknown_send.load(Ordering::SeqCst) {
            return Err(PeerSessionError::OutcomeUnknown);
        }
...
struct Harness {
    ...
    session_event_tx: mpsc::Sender<PeerSessionEvent>,
...
async fn harness(connected: bool) -> Harness {
    harness_with_limits(connected, MessagingServiceLimits::default()).await
}

async fn harness_with_limits(connected: bool, limits: MessagingServiceLimits) -> Harness {
...
async fn outgoing_message_moves_from_pending_to_sent_without_retry() {
    ...
    assert!(matches!(events.recv().await.unwrap(), MessagingEvent::ConversationUpdated { .. }));
...
async fn failed_session_send_is_persisted_as_failed() {
    ...
    assert!(matches!(
        harness
            ...
            .send_message(harness.session_id, harness.peer_id.clone(), "hello".into())
            .await,
        Err(MessagingError::Session(PeerSessionError::ServiceStopped))
    ));
    let messages = harness.handle.snapshot().messages_for_peer(&harness.peer_id);
    assert_eq!(messages.len(), 1);
...
async fn authenticated_receipt_marks_the_matching_outgoing_message_delivered() {
    ...
    harness
        .session_event_tx
        .send(PeerSessionEvent::MessageReceiptReceived {
...
async fn send_requires_the_exact_connected_session_and_peer() {
    let connected_harness = harness(true).await;
    ...
    assert!(matches!(
        connected_harness
            ...
            .await,
        Err(MessagingError::SessionPeerMismatch { .. })
    ));
    assert!(matches!(
        connected_harness
            .handle
            ...
            .await,
        Err(MessagingError::SessionNotConnected { .. })
    ));
    assert!(connected_harness.store.models().is_empty());

    connected_harness.service.shutdown().await.unwrap();

    let negotiating_harness = harness(false).await;
    assert!(matches!(
        negotiating_harness
            .handle
            .send_message(
    ...
    negotiating_harness.service.shutdown().await.unwrap();
}
...
async fn duplicate_incoming_message_is_stored_once_and_acknowledged_each_time() {
    ...
    let sent_at = Utc::now();
    let event = PeerSessionEvent::MessageReceived {
        session_id: harness.session_id,
        peer_id: harness.peer_id.clone(),
        message_id,
        body: "hello".into(),
...
async fn receipt_from_wrong_peer_or_session_cannot_mark_message_delivered() {
    ...
    harness
        .session_event_tx
        .send(PeerSessionEvent::MessageReceived {
            session_id: harness.session_id,
            peer_id: wrong_peer,
...
async fn startup_marks_pending_rows_unknown_instead_of_retrying_them() {
    ...
    let message_id = MessageId::new();
    let peer_id = PeerId::new("peer-a").unwrap();
    store.insert_model(MessageModel {
        id: 1,
        session_id: session_id.to_string(),
        message_id: message_id.to_string(),
        peer_id: peer_id.to_string(),
    ...
    let sessions = Arc::new(FakeSessions {
        snapshot: PeerSessionServiceSnapshot::default(),
        fail_send: AtomicBool::new(false),
        unknown_send: AtomicBool::new(false),
        sent: Mutex::new(Vec::new()),
        receipt_tx,
    ...
    let service = MessagingService::start_with_transport(store, sessions, event_rx).await.unwrap();
    let messages = service.handle().snapshot().messages_for_peer(&peer_id);
    assert_eq!(messages[0].status, MessageStatus::Unknown);
...
async fn expired_queued_command_has_no_late_side_effect() {
    let limits = MessagingServiceLimits {
        command_start_timeout: Duration::from_millis(25),
        shutdown_timeout: Duration::from_secs(1),
    };
    let harness = harness_with_limits(true, limits).await;
    ...
    assert!(matches!(send.await.unwrap(), Err(MessagingError::CommandExpired)));
...
async fn shutdown_rejects_queued_commands_before_they_start() {
    ...
    harness
        ...
        .unwrap();
    let shutdown = tokio::spawn(harness.service.shutdown());
    tokio::task::yield_now().await;
    harness.store.release_list();

    assert!(refresh.await.unwrap().is_ok());
    assert!(matches!(response_rx.await.unwrap(), Err(MessagingError::ServiceStopping)));
...
async fn saturated_command_admission_is_rejected_without_side_effects() {
    ...
    assert!(matches!(
        harness
            ...
            .await,
        Err(MessagingError::ServiceBusy)
    ));
    assert!(harness.store.models().is_empty());
    assert!(harness.sessions.sent.lock().unwrap().is_empty());
...
#[tokio::test]
async fn shutdown_aborts_and_joins_a_stalled_actor_within_its_deadline() {
    let limits = MessagingServiceLimits {
    ...
    let result = tokio::time::timeout(Duration::from_secs(1), harness.service.shutdown())
        .await
        .expect("owner shutdown must be bounded");
    assert!(matches!(result, Err(MessagingError::ShutdownTimeout)));
    assert!(matches!(refresh.await.unwrap(), Err(MessagingError::ResponseDropped)));
}
