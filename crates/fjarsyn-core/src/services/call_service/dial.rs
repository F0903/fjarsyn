...
use super::{CallService, CallState, CallTarget};
use crate::{networking::discovery::PeerInfo, services::contacts_service::Contact};
...
impl CallService {
    pub async fn dial(
        ...
        *self.state.write().unwrap() = CallState::Dialing { target: target.clone(), peer_id: None };
        ...
        *self.state.write().unwrap() =
            CallState::Dialing { target: target.clone(), peer_id: tid.clone() };
