...
use super::{
    super::{
        CodecDirection, CodecDirectionState, CodecOperation, CodecPoison, CodecPoisonReason,
        CodecWorkerError, Error,
    },
    support::{
        BlockingGate, DecoderPlan, EncoderPlan, ReleaseGateOnDrop, ScriptedCodecBackendFactory,
...
async fn returned_creation_and_call_errors_do_not_poison_encoding() {
    let factory = ScriptedCodecBackendFactory::new(
    ...
    assert_eq!(handle.snapshot().encode, CodecDirectionState::Available);

    let mut call_error = handle.open_encoder(encoder_config()).await.unwrap();
    call_error.try_send(test_frame()).unwrap();
    assert!(matches!(call_error.recv().await, Some(Err(CodecWorkerError::Codec(_)))));
    assert_eq!(handle.snapshot().encode, CodecDirectionState::Available);
    drop(call_error);
...
async fn encode_deadline_poison_is_sticky_and_decode_remains_available() {
    ...
    assert!(matches!(
        failure,
        Some(Err(CodecWorkerError::RestartRequired(CodecPoison {
            direction: CodecDirection::Encode,
            operation: CodecOperation::Encode,
            reason: CodecPoisonReason::DeadlineExceeded,
        })))
    ));
    assert!(matches!(handle.snapshot().encode, CodecDirectionState::RestartRequired(_)));
    assert_eq!(handle.snapshot().decode, CodecDirectionState::Available);
...
async fn one_decoder_timeout_cancels_siblings_without_poisoning_encode() {
    ...
    gate.wait_until_started().await;
    assert!(matches!(blocked.recv().await, Some(Err(CodecWorkerError::RestartRequired(_)))));
    assert!(matches!(sibling.recv().await, Some(Err(CodecWorkerError::RestartRequired(_)))));
    assert_eq!(handle.snapshot().encode, CodecDirectionState::Available);
...
async fn constructor_timeout_poison_is_bounded_and_quarantined() {
    ...
    let _release_gate = ReleaseGateOnDrop(gate.clone());
    let factory =
        ScriptedCodecBackendFactory::new(vec![EncoderPlan::CreateBlock(gate.clone())], vec![]);
    let (service, handle) = test_service(factory);
    ...
    assert!(matches!(
        result,
        Err(Error::RestartRequired(CodecPoison {
            direction: CodecDirection::Encode,
            operation: CodecOperation::Initialize,
            reason: CodecPoisonReason::DeadlineExceeded,
        }))
    ));
