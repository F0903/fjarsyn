...
use fjarsyn_core::capture_providers::{
    CaptureProvider, PlatformCaptureItem, PlatformCaptureProvider, user_pick_platform_capture_item,
};
use iced::Task;
...
impl CallScreen {
    pub(super) fn perform_end_call(&self, ctx: AppContext<'_>) -> Task<Message> {
        ...
        let disconnect_task = if let Some(service) = &ctx.runtime.services.call_service {
    ...
    pub(crate) fn set_capture_provider(
        &mut self,
        provider: Arc<tokio::sync::RwLock<PlatformCaptureProvider>>,
    ) -> bool {
