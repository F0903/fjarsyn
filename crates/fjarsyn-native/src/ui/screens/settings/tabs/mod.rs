...
macro_rules! register_tabs {
    ($($($segment:ident)::+),*) => {
        pub(super) static TABS: LazyLock<BTreeMap<&'static str, Arc<dyn SettingsTab>>> = LazyLock::new(|| BTreeMap::from([
            $( (fjarsyn_core::get_type_name!($($segment)::+), Arc::new($($segment)::+) as Arc<dyn SettingsTab>) ),*
        ]));
    }
}
...
macro_rules! settings_tab {
    ($($segment:ident)::+) => {
        $crate::ui::screens::settings::tabs::get_tab(fjarsyn_core::get_type_name!($($segment)::+)).unwrap()
    };
}
