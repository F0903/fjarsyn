...
use crate::ui::{
    ...
    runtime::MediaProjection,
    theme,
};

pub(super) fn restart_required_banner<'a>(media: &MediaProjection) -> Option<Element<'a, Message>> {
    let copy =
        restart_required_copy(media.encoder_restart_required(), media.decoder_restart_required())?;
